...
class DB(Base):
    ...
    def update_stock_basic_info(cls,  ts_code, name, area, industry, market, curr_type, list_status, list_date, delist_date, is_hs):
        pd.io.sql.execute('update stock_basic set '
                          'name=%s, area=%s, industry=%s, market=%s, curr_type=%s, '
                          'list_status=%s, list_date=%s, delist_date=%s, is_hs=%s where ts_code=%s',
                          cls.engine, params=[name, area, industry, market, curr_type, list_status, list_date, delist_date, is_hs, ts_code])
    ...
    def get_code_info(cls, code_id='', start_date='', end_date='', period='', TTB='daily'):
        ...
        if start_date == '':
            code_info = pd.read_sql(
                sa.text(
                    ' SELECT tc.cal_date, d.*, af.adj_factor, db.turnover_rate_f, db.float_share, db.total_mv, db.pe, db.pb'
                    ' FROM  ' + TTB + ' d '
    ...
    @classmethod
    def get_existed_fx(cls, table_name, fx_id, start_date, end_date):
        existed_codes = pd.read_sql(
            sa.text(
                ' SELECT tc.cal_date FROM ' + table_name + ' as api '
                ' left join trade_cal tc on tc.id = api.date_id'
                ' where api.fx_id = :fx_id and tc.cal_date between :start_date and :end_date'),
            cls.engine,
            params={'fx_id': fx_id, 'start_date': start_date, 'end_date': end_date}
        )
        return existed_codes
