...
#     # code_ids = [2352, 1587, 1348, 2119, 1376, 3304, 1383, 2074, 2057, 3394]
    ...
    for code_id in code_ids:
        ...
        base = trade_cal.join(logs[['date_id', 'end_date',  'LLP', 'LP', 'MP', 'HP', 'HHP', 'MP_pct',
                                    'roe','roe_rd', 'roe_mv', 'roe_mv_ci0', 'roe_mv_ci1', 'roe_sale_mv', 'report_adj_factor', 'rev_pct_yearly',
        ...
        data['roe'] = base['roe']
        data['roe_rd'] = base['roe_rd']
        data['roe_mv'] = base['roe_mv']
        data['roe_mv_ci0'] = base['roe_mv_ci0']
        data['roe_mv_ci1'] = base['roe_mv_ci1']
        ...
        data['pp'] = round(base['V'] * accrued_interest / pb, 2)
        data['pp_ci0'] = round(base['V_ci0'] * accrued_interest / pb, 2)
        data['pp_ci1'] = round(base['V_ci1'] * accrued_interest / pb, 2)
        data['pp_adj'] = round(base['V_adj'] * accrued_interest / pb, 2)
        data['pp_rd'] = round(base['V_rd'] * accrued_interest / pb, 2)
        data['pp_sale'] = round(base['V_sale'] * accrued_interest / pb, 2)
        data['pp_ebitda'] = round(base['V_ebitda'] * accrued_interest / pb, 2)
        ...
        print('3')
        adj_MP = round(adj_close * data['pp'], 2)
        ...
        MP_ci0 = round(adj_close * data['pp_ci0'], 2)
        MP_ci1 = round(adj_close * data['pp_ci1'], 2)
        data['adj_LLP'] = MP_ci0
        data['adj_HHP'] = MP_ci1

        # print(pd.concat([MP_ci1, adj_close, data['pp_ci1'], accrued_interest, base['roe_mv'], d_dates],axis=1))
        # os.ex
        ...
        for i in range(len(base)):
            ...
            if (data.iloc[i]['pp'] + data.iloc[i]['pp_rd']) > 3 and data.iloc[i]['pp_adj'] > 1 and data.iloc[i]['dpd_RR'] > 1.7:
                nice.iloc[i] = 1
            elif (data.iloc[i]['pp'] + data.iloc[i]['pp_rd']) < 1 and data.iloc[i]['pp_adj'] < 0.5 and data.iloc[i]['dpd_RR'] < 1:
                nice.iloc[i] = -1
            else:
                nice.iloc[i] = 0
