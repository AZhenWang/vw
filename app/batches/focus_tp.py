...
def execute(start_date='', end_date=''):
    ...
    for code_id in code_ids:
        ...
        Y = DB.get_code_info(code_id=code_id, end_date=end_date)
        Y = Y['close'] * Y['adj_factor']
        Y.name = 'close'
        ...
        for i in range(data_len-cal_length, data_len):
            date_id = Y.index[i]
            DB.delete_recommend_log(code_id=code_id, date_id=date_id, recommend_type='tp')

            predict_Y = tp_model.run(y=Y, predict_time=60)
