...
def execute(start_date='', end_date=''):
    ...
    # code_ids = range(3592, 3899)
    # code_ids = range(1, 3899)
    # code_ids = [3304, 2344, 1376, 1348, 2057, 2074]
    # code_ids = [3590, 3591, 3592]
    code_ids = range(3591, 3982)
    # code_ids = range(1, 3982)
    # code_ids = [1348,1376, 3304, 1383, 913, 2777]
    ...
    for code_id in code_ids:
        ...
        for j in range(len(logs)):
            ...
            if j >= 1:
                ...
                pre_cash_act_in = logs.iloc[j - 1]['cash_act_in']
                pre_cash_act_out = logs.iloc[j - 1]['cash_act_out']
            else:
                pre_roe = log['roe_mv']
                pre_total_turn = log['total_turn']
                pre_gross_rate = log['gross_rate']
                pre_cash_act_in = log['cash_act_in']
                pre_cash_act_out = log['cash_act_out']
            ...
            if log['roe'] - pre_roe > -1 \
                    ...
                    and log['cash_act_in'] > pre_cash_act_in\
                    and log['cash_act_out'] > pre_cash_act_out:
